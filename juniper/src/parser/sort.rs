    ...
    where T: Text<'a>
{
    ...
    value.definitions.sort_by_key(|f| match f {
        ...
        Definition::TypeDefinition(x) => format!("3:{}", match x {
        ...
        Definition::TypeExtension(x) => format!("4:{}", match x {
...
fn schema_definition<'a, T>(value: &mut SchemaDefinition<'a, T>) where T: Text<'a> {
...
fn type_definition<'a, T>(value: &mut TypeDefinition<'a, T>) where T: Text<'a> {
    match value {
...
fn type_extension<'a, T>(value: &mut TypeExtension<'a, T>) where T: Text<'a> {
    match value {
...
fn directive_definition<'a, T>(value: &mut DirectiveDefinition<'a, T>) where T: Text<'a> {
...
fn object_type<'a, T>(value: &mut ObjectType<'a, T>) where T: Text<'a> {
    value.implements_interfaces.sort();
    fields(&mut value.fields);
}

fn object_type_extension<'a, T>(value: &mut ObjectTypeExtension<'a, T>) where T: Text<'a> {
    value.implements_interfaces.sort();
    fields(&mut value.fields);
}

fn fields<'a, T>(value: &mut Vec<Field<'a, T>>) where T: Text<'a> {
...
fn interface_type<'a, T>(value: &mut InterfaceType<'a, T>) where T: Text<'a> {
    fields(&mut value.fields);
}

fn interface_type_extension<'a, T>(value: &mut InterfaceTypeExtension<'a, T>) where T: Text<'a> {
    fields(&mut value.fields);
}


fn union_type<'a, T>(value: &mut UnionType<'a, T>) where T: Text<'a> {
    value.types.sort()
}

fn union_type_extension<'a, T>(value: &mut UnionTypeExtension<'a, T>) where T: Text<'a> {
    value.types.sort()
}

fn enum_type<'a, T>(value: &mut EnumType<'a, T>) where T: Text<'a> {
    value.values.sort_by_key(|x| x.name.clone())
}

fn enum_type_extension<'a, T>(value: &mut EnumTypeExtension<'a, T>) where T: Text<'a> {
    value.values.sort_by_key(|x| x.name.clone())
}

fn input_object_type<'a, T>(value: &mut InputObjectType<'a, T>) where T: Text<'a> {
    value.fields.sort_by_key(|x| x.name.clone())
}

fn input_object_type_extension<'a, T>(value: &mut InputObjectTypeExtension<'a, T>) where T: Text<'a> {
    value.fields.sort_by_key(|x| x.name.clone())
}